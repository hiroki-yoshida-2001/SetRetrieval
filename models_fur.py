...
class SMN(tf.keras.Model):
    ...
    def train_step(self,data):
        ...
        with tf.GradientTape() as tape:
            ...
            if self.is_L2_norm_loss:
                ...
                l2_loss = 0
            if not self.set_loss:
                loss = self.compiled_loss(y_pred = cos_sim_forloss, y_true = c_label, regularization_losses=self.losses) + style_loss * (1/20) + setMatchingloss * 3.0 + l2_loss
        ...
        if not self.set_loss:
            ...
            # return metrics as dictionary
            return {'cos_sim_loss': loss-style_loss*(1/20)-setMatchingloss*3.0 -l2_loss, 'Match_loss': setMatchingloss*3.0, 'L2_loss': l2_loss,'Set_accuracy':self.metrics[1].result()}
        else:
    ...
    def predict_step(self,data):
        ...
        y_test = y
        y_test = data[1]
        # x = {x, x_size, c_label}, y_true : set label to identify positive pair. (nSet, )
        if len(x) == 2:
            x, x_size = x
        else:
            x, x_size, c_label, item_id = x 
        ...
        # x = (x-self.whitening_mean)/ self.whitening_std
        if not self.set_loss:
            # c_label (before): odd and even number index hasn't been shuffuled => c_label(after): each index indicates the answer
            ans_c_label = tf.gather(c_label, tf.where(tf.equal(y_true,1))[:,1])
            pred_size = tf.gather(x_size, tf.where(tf.equal(y_true,1))[:,1])
        ...
        # else:
        #     gallery = (gallery - self.whitening_mean) / self.whitening_std # normalization
        # predict
        # predSMN : (nSet, nItemMax, d)
        ...
        if len(x) <= 50:
            return predSMN, gallery, Real_score, Fake_score
        else:
            return x, x_size, c_label, y_test, ans_c_label, predSMN, gallery, item_id
#----------------------------